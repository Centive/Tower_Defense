	...
	, mGenerations(nullptr)
{
	...
	{
		*((s32*)(mData + i)) = (s32)i + 1;				//Assigns an index for the next slot
	}
	*((s32*)(mData + capacity - 1)) = -1;				//The last position of the slot is indexed as -1
														//so we will know that it that is the last slot

	// Every slot starts at generation 0
	mGenerations = (u16*)malloc(capacity * sizeof(u16));//Once a handler is created 
